...
let startX = 5; // Starting X position for Tetromino
let startY = 0; // Starting Y position for Tetromino
let score = 0; // Tracks the score
let level = 1; // Tracks current level
let levelUp = 100; // Tracks current level
let speed = 1; // set speed to 1
let timer;  // set timer variable to clear interval
let winOrLose = "Playing";
let isPaused = false;
let isMuted = false;
let backgroundSound = document.getElementById('bgMusic');
let gameOver = new Audio('./sounds/game over.mp3');
let rotate = new Audio('./sounds/rotate.mp3');
let lineClear = new Audio('./sounds/line clear.mp3');
let hit = new Audio('./sounds/hit.mp3');
let hardDrop = new Audio('./sounds/hard drop.mp3');
...
let tetrominoColors = ['purple', 'cyan', 'blue', 'brown', 'orange', 'green', 'red'];
...
// 5. Array for storing stopped shapes
...
function SetupCanvas() {
    ...
    backgroundSound = document.getElementById('bgMusic');
    ...
    ctx.strokeRect(300, 35, 160, 25);
    ctx.fillText(score.toString(), 305, 53);

    // Draw level
    ctx.fillText("LEVEL", 300, 90);
    ctx.strokeRect(300, 95, 160, 25);
    ctx.fillText(level.toString(), 305, 113);

    // Draw next block
    ctx.fillText("POINTS TABLE", 300, 153);
    ctx.strokeRect(300, 158, 160, 120);
    ctx.font = '15px Arial';
    ctx.fillText("1 line   = 10  POINTS", 305, 188);
    ctx.fillText("2 lines = 30  POINTS", 305, 210);
    ctx.fillText("3 lines = 50  POINTS", 305, 232);
    ctx.fillText("4 lines = 100 POINTS", 305, 254);

    // // Draw controls
    ctx.fillText("CONTROLS", 300, 310);
    ctx.strokeRect(300, 315, 160, 155);
    ctx.font = '19px Arial';
    ctx.fillText(" ← :  Left", 305, 340);
    ctx.fillText(" → :  Right", 305, 363);
    ctx.fillText("  ↓  :  Down", 305, 388);
    ctx.fillText("  ↑  :  Rotate", 305, 413);
    ctx.fillText("  M :  Mute", 305, 438);
...
function CheckForCompletedRows() {
    ...
    if (rowsToDelete == 1) {
        ...
        // update score
        ctx.fillStyle = 'white';
        ctx.fillRect(301, 36, 155, 23);
        ctx.fillStyle = 'black';
        ctx.fillText(score.toString(), 305, 55);
        ...
        if (score % levelUp == 0) {
            ...
            ctx.fillRect(301, 96, 155, 23);
            // Draw level
            ctx.fillStyle = 'black';
            ctx.fillText(level.toString(), 305, 115);
            clearInterval(timer);
            setSpeed(speed);
        }    
    ...
    if (!isMuted)
    {
	if (rowsToDelete > 0) lineClear.play();
    }
}
